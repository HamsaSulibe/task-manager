...
class Task:
    ...
    def __init__(self, title: str, start_date: datetime, due_date: datetime, status=PENDING_STATE) -> None:
        ...
        Args:
            ...
            start_date (datetime): The start date for the task.
    ...
    def list_task(self) -> None:
        ...
        if not visible_tasks:
            ...
            for i, task in enumerate(visible_tasks, 1):
                logging.info(
                    f"{i}. {task.title} | Start: {task.start_date.strftime('%Y-%m-%d')} | "
                    f"Due: {task.due_date.strftime('%Y-%m-%d')} | Needed Time: {task.needed_time} days | "
...
def main() -> None:
    ...
    while True:
        ...
        try:
            ...
            choice = int(cleaned)  # convert to int
        except ValueError:
